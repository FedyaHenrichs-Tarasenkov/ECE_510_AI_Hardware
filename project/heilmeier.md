...
Currently, HDC is run in software on standard microcontrollers. My profiling data (project_profile.txt) proves that the vector binding operation accounts for >90% of total runtime, exposing severe memory bandwidth limits. A standard CPU becomes heavily memory-bound because it has to constantly shuttle massive 10,000-element arrays back and forth from external DRAM just to perform incredibly simple operations (like XOR), resulting in slow execution and wasted power.
